...
void LMT70_Config(void)
{
	...
	GPIO_SetBits(GPIOB,GPIO_Pin_2);	//开启LMT70
	Adc_Init();
}
//初始化ADC
//这里我们仅以规则通道为例
...
{    
  ...
  ADC_Init(ADC1, &ADC_InitStructure);
	...
	ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 1, ADC_SampleTime_480Cycles );	//ADC16,ADC通道,480个周期,提高采样时间可以提高精确度		
...
short Get_Temprate(void)
{
	...
	float V_TAO;	//LMT70采样出的数据
	short result;
 	double temperate;
	V_TAO=Get_Adc_Average(ADC_Channel_5,10);	//读取通道5LMT70,10次取平均
	V_TAO=(float)V_TAO*(3.3/4096)*1000;		//电压值
